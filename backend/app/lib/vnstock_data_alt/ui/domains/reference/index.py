...
import pandas as pd
from app.lib._vnstock_shared.common import indices as shared_indices
...
class IndexDetail:
	...
	def info(A):'Get complete information for this index.';return shared_indices.get_index_info(A.symbol)
	def description(A):'Get description for this index.';return shared_indices.get_index_description(A.symbol)
class IndexReference(BaseDomain):
	...
	def list_by_group(E,group='VN30'):
		...
		if isinstance(A,str):
			D=A.upper().strip()
			if A in shared_indices.get_all_index_groups():return shared_indices.get_indices_by_group(A)
			for C in shared_indices.get_all_index_groups():
				if D in C.upper():return shared_indices.get_indices_by_group(C)
		return pd.DataFrame()
