...
from app.db.models import StockCompany, StockCompanyDataCache
...
class CompanyDataSyncService:
    """Background synchronization for company datasets."""

    DATA_TYPES = (
        CompanyService.DATA_TYPE_OVERVIEW,
    ...
    async def stop_background_tasks(self) -> None:
        """Cancel running company sync task on shutdown."""
        ...
        async with self._sync_lock:
            if self._sync_task and not self._sync_task.done():
                if not force_restart:
                    return {
                        "started": False,
                        "message": "Company sync is already running",
                        "state": "running",
                    }
    ...
    async def _run_sync(self, symbols: Optional[List[str]], quick_sync: bool = False) -> None:
        ...
        if total == 0:
            company_sync_logger.info("Company sync completed immediately (no symbols)")
            sync_status.complete_company_sync(success=True)
            return

        await self._reset_sync_pacer()
        company_sync_logger.info(
            "Company sync runtime config: max_workers=%s target_rpm=%s fixed_wait=%.1fs max_wait=%.1fs",
            ...
            while True:
                ...
                try:
                    async with progress_lock:
                        sync_status.update_company_sync_progress(
                    ...
                    symbol_failed = True
                    company_sync_logger.error(
                        f"Company sync worker {worker_id} failed symbol {symbol}: {e}"
                    )
                finally:
        ...
        try:
            ...
            if failure_count > 0:
                company_sync_logger.warning(
                    "Company sync completed with failures: success=%s failed=%s",
                    success_count,
                    failure_count,
                )
                sync_status.complete_company_sync(
                    success=True,
                    error=f"Company sync completed with {failure_count} failed symbols",
                )
            else:
                company_sync_logger.info(
                    "Company sync completed successfully: success=%s failed=%s",
                    success_count,
                    failure_count,
                )
                sync_status.complete_company_sync(success=True)
            ...
            company_sync_logger.warning("Company sync cancelled")
            sync_status.complete_company_sync(success=False, error="Company sync cancelled")
            raise
        except Exception as e:
            company_sync_logger.error("Company sync failed: %s", e)
            sync_status.complete_company_sync(success=False, error=str(e)[:500])
    ...
    async def _run_company_fetch_with_retry(
        self,
        symbol: str,
        data_type: str,
    ) -> int:
        ...
        while True:
            ...
            try:
                await self._company.refresh_company_dataset(
                    symbol=symbol,
                    data_type=data_type,
                ...
                company_sync_logger.debug(
                    "Company sync dataset synced for %s (%s) attempt=%s elapsed=%.2fs",
                    symbol,
                    data_type,
                    attempt,
                    elapsed,
                ...
                if not (_is_rate_limit_error(e) or isinstance(e, CircuitOpenError)):
                    company_sync_logger.debug(
                        "Company sync dataset failed for %s (%s) attempt=%s elapsed=%.2fs error=%s",
                        symbol,
                        data_type,
                        attempt,
                        elapsed,
                ...
                company_sync_logger.warning(
                    "Company sync rate-limit pause symbol=%s data_type=%s attempt=%s "
                ...
                if (
                    ...
                    company_sync_logger.error(
                        "Company sync max rate-limit wait exceeded symbol=%s data_type=%s "
    ...
    async def _sync_symbol(self, symbol: str) -> None:
        for data_type in self.DATA_TYPES:
            ...
            if retries > 0:
                company_sync_logger.debug(
                    "Company sync dataset completed after retries for %s (%s): retries=%s",
                    symbol,
                    data_type,
                    retries,
                )
    ...
    def _fetch_listing_symbols_sync(self) -> List[str]:
        ...
        try:
            ...
            company_sync_logger.warning(f"Error fetching listing symbols for company sync: {e}")
    ...
    async def _fetch_db_symbols(self) -> List[str]:
        async with async_session() as session:
            ...
            cache_stmt = select(StockCompanyDataCache.symbol)
            cache_result = await session.execute(cache_stmt)
            cache_symbols = [self._normalize_symbol(row[0]) for row in cache_result.all() if row[0]]

        return list(dict.fromkeys([*company_symbols, *cache_symbols]))
    ...
    async def _filter_quick_sync_symbols(self, symbols: List[str]) -> List[str]:
        ...
        company_sync_logger.info(
            "Company quick sync filter applied: input=%s skipped_fully_synced=%s queued=%s",
            len(symbols),
    ...
    async def _fetch_fully_synced_symbols(self, symbols: List[str]) -> set[str]:
        async with async_session() as session:
            stmt = (
                select(StockCompanyDataCache.symbol)
                .where(
                    StockCompanyDataCache.symbol.in_(symbols),
                    StockCompanyDataCache.data_type.in_(self.DATA_TYPES),
                )
                .group_by(StockCompanyDataCache.symbol)
                .having(func.count(distinct(StockCompanyDataCache.data_type)) == len(self.DATA_TYPES))
            )
